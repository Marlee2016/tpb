...
class IndexController extends Controller
{
    ...
    public function index()
    {
        ...
        $show = $p->show();
        $list = $q->where('active=1')->order('id DESC')->page($now . ',' . $per)->select();
    ...
    public function edit($id)
    {
        ...
        if (IS_POST) {
            ...
            if ((M('Questions')->where("id=$id")->save($data)) >= 0) {
                $this->redirect("Index/edit?id=$id", [], 1, '<h1>ok</h1>');
            }
        }
    ...
    public function destroy($id)
    {
        ...
        if (M('Questions')->where("id=$id")->save(['active' => -1])) {
            $this->redirect("Index/index", [], 1, '<h1>ok</h1>');
        } else {
            dd(M('Questions')->getError());
        }
